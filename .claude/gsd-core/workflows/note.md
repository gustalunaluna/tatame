...
- **Global scope**: `/workspace/tatame/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspace/tatame/.claude/notes/`)
...
2. Glob `/workspace/tatame/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/workspace/tatame/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/workspace/tatame/.claude/notes/` — works in any directory
